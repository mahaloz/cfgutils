...
class BlockMatcherBase:
    ...
    def analyze(self):
        ...
        for b1, b2_scores in sim_map.items():
            ...
            best_match = max(b2_scores, key=b2_scores.get)
    ...
    def generate_similarities(self) -> Dict[object, Dict[object, float]]:
        ...
        for b1, b2 in itertools.product(self._g1_nodes, self._g2_nodes):
            scores[b1][b2] = self.block_similarity(b1, b2)

        return scores

    def block_similarity(self, b1, b2) -> float:
        feat_func_map = {
        ...
        for feat_func, weight in feat_func_map.items():
            f1, f2 = feat_func(b1), feat_func(b2)
    ...
    def get_number_of_arithmetic_ins(self, block) -> int:
        raise NotImplementedError()

    def get_number_of_calls(self, block) -> int:
        raise NotImplementedError()

    def get_number_of_ins(self, block) -> int:
        raise NotImplementedError()

    def get_number_of_logic_ins(self, block) -> int:
        raise NotImplementedError()

    def get_number_of_branch_ins(self, block) -> int:
        raise NotImplementedError()

    def get_str_consts(self, block) -> List[str]:
        raise NotImplementedError()

    def get_num_consts(self, block) -> List[int]:
        raise NotImplementedError()
