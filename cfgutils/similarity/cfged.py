...
def addr_to_node_map(graph):
    # TODO: this is bugged, you can't list nodes by addr,
    # there needs to also be an index for nodes that share the same addr
    return {
...
def find_dst_mismatches(
    ...
    for dec_src, dec_dst in dec_edges:
        ...
        src_src_addrs = set(
            itertools.chain.from_iterable([src_addr_to_line_map.get(addr, set()) for addr in dec_src_addrs]))
...
def destroy_old_region(cfg: nx.DiGraph, expanded_region_graph: nx.DiGraph, r_head: GenericBlock):
    ...
    merged_node = r_head.merge_many_blocks(r_head.addr, r_nodes)
...
def expand_region_to_successor_block_graph(
...
) -> Tuple[Optional[nx.DiGraph], Optional[GenericBlock]]:
    ...
    if include_succs and region.successors and region.graph_with_successors:
        ...
        if isinstance(successor, GenericBlock):
...
def find_some_tail_region(region: Union[GenericBlock, GraphRegion], node_blacklist, og_cfg: nx.DiGraph) -> Optional[
    Union[GraphRegion, GenericBlock]]:
    # sanity check
...
def find_matching_source_region(
    ...
    if len(matching_src_regions) > 1:
        ...
        for (head, (region, size)) in matching_src_regions:
            if not isinstance(head, GenericBlock):
                continue

            has_merged_region = head.is_merged_node
...
def fast_switch_region_ged(expanded_dec_r_graph, expanded_src_r_graph):
    ...
    return abs(len(expanded_dec_r_graph.nodes) - len(expanded_src_r_graph.nodes)) + abs(
        len(expanded_dec_r_graph.edges) - len(expanded_src_r_graph.edges))
...
def cfg_edit_distance(
    ...
    # the max possible GED score for any two graphs
    ...
    while True:
        ...
        elif isinstance(dec_region, GenericBlock):
        ...
        if too_large_for_exact:
            ...
            if size_diff > dec_size * 2 or size_diff > src_size * 2:
                l.debug(
                    f"Difference in regions size too large to approximate (Dec, Src): ({dec_size}, {src_size}), skipping for now...")
                region_blacklist.add(dec_r_head.addr)
        ...
        switch_region_case = is_switch_region(expanded_src_graph) and is_switch_region(
            expanded_dec_graph) and too_large_for_exact
        ...
        extra_dec_edges = destroy_old_region(
            dec_cfg,
            expand_region_to_block_graph(dec_region, dec_cfg) if not isinstance(dec_region, GenericBlock) else dec_region,
            expand_region_head_to_block(dec_region) if not isinstance(dec_region, GenericBlock) else dec_region
        )

        # extra edge diffs are only needed in the event we can't use a normally expanded region graph
        if bad_successor_case:
            edge_diff = find_dst_mismatches(extra_dec_edges, extra_src_edges, dec_line_to_addr_map,
                                            src_addr_to_line_map)
            if edge_diff:
