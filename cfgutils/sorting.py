...
def find_merge_points(function_addr, function_endpoints, graph):  # pylint:disable=unused-argument
    ...
    ordered_merge_points = quasi_topological_sort_nodes(graph, merge_points)
...
def quasi_topological_sort_nodes(
    ...
    for src, dst in graph.edges():
        scc_index = _components_index_node(sccs, src)
        if scc_index is not None:
            src = SCCPlaceholder(scc_index)
        scc_index = _components_index_node(sccs, dst)
        if scc_index is not None:
    ...
    for n in tmp_nodes:
        if isinstance(n, SCCPlaceholder):
            _append_scc(graph, ordered_nodes, sccs[n.scc_id], loop_heads=loop_heads)
        else:
